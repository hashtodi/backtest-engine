"""Streamlit form + runner for the Triple-SuperTrend + EMA9/21 credit-spread backtest."""
import json
import math
import os
from datetime import date

import pandas as pd
import streamlit as st

from engine.triple_st_ema_spread_backtest import run


DEFAULT_CONFIG_PATH = "saved_strategies/triple_st_ema_spread.json"
DEFAULT_OPTIONS_PATH = "data/options/nifty/NIFTY_OPTIONS_1m.parquet"
DEFAULT_SPOT_PATH = "data/spot/nifty/NIFTY_1m.parquet"
DEFAULT_OUTPUT_DIR = "output/triple_st_ema_spread"


def _load_default_config() -> dict:
    if os.path.exists(DEFAULT_CONFIG_PATH):
        with open(DEFAULT_CONFIG_PATH) as f:
            return json.load(f)
    raise FileNotFoundError(f"Missing config file at {DEFAULT_CONFIG_PATH}.")


def _fmt_money(val: float) -> str:
    if val is None or (isinstance(val, float) and math.isnan(val)):
        return "n/a"
    return f"Rs {val:,.0f}"


def _fmt_pct(val: float) -> str:
    if val is None or (isinstance(val, float) and math.isnan(val)):
        return "n/a"
    return f"{val * 100:.2f}%"


def render_triple_st_ema_spread_backtest() -> None:
    st.header("Triple SuperTrend + EMA9/21 Cross - Credit Spread")
    st.caption(
        "Regime from 3m/5m/10m SuperTrend alignment on NIFTY spot (continuous "
        "across days, lookahead-off). All three GREEN + EMA9 crosses ABOVE "
        "EMA21 -> BULL PUT: sell ATM-2 PE / buy ATM-6 PE. All three RED + EMA9 "
        "crosses BELOW EMA21 -> BEAR CALL: sell ATM+2 CE / buy ATM+6 CE. The "
        "cross is evaluated at the 1-min bar's close; ATM, strikes and premiums "
        "are taken from that same close (no next-minute delay). Exits are "
        "absolute INR per lot on the live spread (TP/SL), detected on each 1-min "
        "option close and filled at the NEXT 1-min OPEN; EOD square-off at the "
        "deadline. One spread open at a time, unlimited re-entries per day. On "
        "weekly expiry days the strategy rolls to the next weekly expiry (code 2)."
    )

    cfg = _load_default_config()
    signal_cfg = cfg.get("signal", {}) or {}
    entry_cfg = cfg.get("entry", {}) or {}
    exit_cfg = cfg.get("exit", {}) or {}
    structure_cfg = cfg.get("structure", {}) or {}
    expiry_cfg = cfg.get("expiry", {}) or {}

    col_a, col_b = st.columns(2)
    start_date = col_a.date_input(
        "Backtest start", value=date.fromisoformat(cfg["backtest_start"]),
        key="tse_start",
    )
    end_date = col_b.date_input(
        "Backtest end", value=date.fromisoformat(cfg["backtest_end"]),
        key="tse_end",
    )

    st.markdown("**SuperTrend (multi-timeframe regime)**")
    col_c, col_d, col_e = st.columns(3)
    st_factor = col_c.number_input(
        "SuperTrend factor", min_value=0.1, max_value=20.0, step=0.1,
        value=float(signal_cfg.get("st_factor", 3.0)),
        key="tse_st_factor",
    )
    st_atr_period = col_d.number_input(
        "SuperTrend ATR period", min_value=1, max_value=100, step=1,
        value=int(signal_cfg.get("st_atr_period", 12)),
        key="tse_st_atr",
    )
    warmup_days = col_e.number_input(
        "Warm-up days", min_value=1, max_value=60, step=1,
        value=int(signal_cfg.get("warmup_days", 10)),
        help="Calendar days loaded before the start to seed SuperTrend/EMA.",
        key="tse_warmup",
    )

    htf_modes = ["rolling", "anchored"]
    default_mode = str(signal_cfg.get("htf_mode", "rolling")).lower()
    htf_mode = st.radio(
        "Higher-timeframe SuperTrend mode",
        options=htf_modes,
        index=htf_modes.index(default_mode) if default_mode in htf_modes else 0,
        horizontal=True,
        help="rolling = trailing N-min window recomputed every minute (faster, "
             "smoother). anchored = fixed 09:15-anchored N-min bars that step "
             "only at boundaries (TradingView lookahead_off).",
        key="tse_htf_mode",
    )

    col_f, col_g, col_h = st.columns(3)
    tf1_min = col_f.number_input(
        "Timeframe 1 (min)", min_value=1, max_value=60, step=1,
        value=int(signal_cfg.get("tf1_min", 3)), key="tse_tf1",
    )
    tf2_min = col_g.number_input(
        "Timeframe 2 (min)", min_value=1, max_value=60, step=1,
        value=int(signal_cfg.get("tf2_min", 5)), key="tse_tf2",
    )
    tf3_min = col_h.number_input(
        "Timeframe 3 (min)", min_value=1, max_value=120, step=1,
        value=int(signal_cfg.get("tf3_min", 10)), key="tse_tf3",
    )

    st.markdown("**EMA cross (1-min trigger)**")
    col_i, col_j = st.columns(2)
    ema_fast = col_i.number_input(
        "EMA fast", min_value=1, max_value=200, step=1,
        value=int(signal_cfg.get("ema_fast", 9)), key="tse_ema_fast",
    )
    ema_slow = col_j.number_input(
        "EMA slow", min_value=2, max_value=400, step=1,
        value=int(signal_cfg.get("ema_slow", 21)), key="tse_ema_slow",
    )

    st.markdown("**Entry window & structure**")
    col_k, col_l, col_m = st.columns(3)
    window_start = col_k.text_input(
        "Entry window start (HH:MM)",
        value=str(entry_cfg.get("window_start", "09:30")), key="tse_win_start",
    )
    window_end = col_l.text_input(
        "Entry window end (HH:MM)",
        value=str(entry_cfg.get("window_end", "14:45")), key="tse_win_end",
    )
    lots = col_m.number_input(
        "Lots", min_value=1, step=1,
        value=int(structure_cfg.get("lots", 1)), key="tse_lots",
    )

    col_n, col_o, col_p = st.columns(3)
    sell_offset_abs = col_n.number_input(
        "Sell-leg offset (|strikes| from ATM)", min_value=1, max_value=20, step=1,
        value=int(structure_cfg.get("sell_offset_abs", 2)),
        help="2 strikes = 100 pts OTM.", key="tse_sell_off",
    )
    buy_offset_abs = col_o.number_input(
        "Buy-leg offset (|strikes| from ATM)", min_value=1, max_value=20, step=1,
        value=int(structure_cfg.get("buy_offset_abs", 6)),
        help="6 strikes = 300 pts OTM. Must be further OTM than the sell leg.",
        key="tse_buy_off",
    )
    max_trades = col_p.number_input(
        "Max trades per day (0 = unlimited)", min_value=0, max_value=50, step=1,
        value=int(structure_cfg.get("max_trades_per_day", 0)), key="tse_max_trades",
    )

    st.markdown("**Exit (absolute INR per lot)**")
    col_q, col_r, col_s = st.columns(3)
    tp_inr = col_q.number_input(
        "Take-profit per lot (Rs, 0 = off)", min_value=0.0, step=50.0,
        value=float(exit_cfg.get("tp_inr", 800.0)),
        help="Close when live spread P&L >= this x lots.", key="tse_tp",
    )
    sl_inr = col_r.number_input(
        "Stop-loss per lot (Rs, 0 = off)", min_value=0.0, step=50.0,
        value=float(exit_cfg.get("sl_inr", 650.0)),
        help="Close when live spread P&L <= -this x lots.", key="tse_sl",
    )
    square_off_time = col_s.text_input(
        "EOD square-off (HH:MM)",
        value=str(exit_cfg.get("square_off_time", "15:15")), key="tse_square_off",
    )

    capital = st.number_input(
        "Reference capital (Rs)", min_value=1, step=10000,
        value=int(cfg["sizing"]["reference_capital"]), key="tse_capital",
    )

    if st.button("Run backtest", type="primary", key="tse_run_button"):
        if buy_offset_abs <= sell_offset_abs:
            st.error("Buy-leg offset must be greater than the sell-leg offset.")
            return
        run_config = dict(cfg)
        run_config["backtest_start"] = start_date.isoformat()
        run_config["backtest_end"] = end_date.isoformat()
        run_config["signal"] = {
            **signal_cfg,
            "st_factor": float(st_factor),
            "st_atr_period": int(st_atr_period),
            "htf_mode": str(htf_mode),
            "tf1_min": int(tf1_min),
            "tf2_min": int(tf2_min),
            "tf3_min": int(tf3_min),
            "ema_fast": int(ema_fast),
            "ema_slow": int(ema_slow),
            "warmup_days": int(warmup_days),
        }
        run_config["entry"] = {
            **entry_cfg,
            "window_start": str(window_start),
            "window_end": str(window_end),
        }
        run_config["structure"] = {
            **structure_cfg,
            "lots": int(lots),
            "sell_offset_abs": int(sell_offset_abs),
            "buy_offset_abs": int(buy_offset_abs),
            "max_trades_per_day": int(max_trades),
        }
        run_config["exit"] = {
            **exit_cfg,
            "tp_inr": float(tp_inr),
            "sl_inr": float(sl_inr),
            "square_off_time": str(square_off_time),
        }
        run_config["expiry"] = {
            **expiry_cfg,
            "expiry_type": "WEEK",
            "expiry_roll": True,
        }
        run_config["sizing"] = {**cfg["sizing"], "reference_capital": int(capital)}

        with st.spinner("Running backtest..."):
            result = run(
                run_config,
                options_path=DEFAULT_OPTIONS_PATH,
                output_dir=DEFAULT_OUTPUT_DIR,
                spot_path=DEFAULT_SPOT_PATH,
            )

        summary = result["summary"]

        st.subheader("Summary")
        m1, m2, m3, m4 = st.columns(4)
        m1.metric(
            "Trades",
            f"{summary['total_trades']} over {result['days_processed']} days",
        )
        m2.metric("Win rate", _fmt_pct(summary["win_rate"]))
        m3.metric("Total P&L", _fmt_money(summary["total_pnl_inr"]))
        m4.metric("Total return on capital", _fmt_pct(summary["total_return_pct"]))

        m5, m6, m7, m8 = st.columns(4)
        m5.metric(
            "Max drawdown", _fmt_money(summary["max_drawdown_inr"]),
            delta=_fmt_pct(-summary["max_drawdown_pct"]),
            delta_color="inverse",
        )
        m6.metric("Max consec losses", str(summary["max_consecutive_losses"]))
        m7.metric("Best trade", _fmt_money(summary["best_trade_inr"]))
        m8.metric("Worst trade", _fmt_money(summary["worst_trade_inr"]))

        m9, m10, m11, m12 = st.columns(4)
        m9.metric("Mean P&L", _fmt_money(summary["mean_pnl_inr"]))
        m10.metric("Median P&L", _fmt_money(summary["median_pnl_inr"]))
        m11.metric("LONG (bull-put)", str(summary["long_trades"]))
        m12.metric("SHORT (bear-call)", str(summary["short_trades"]))

        st.write("**Exit reasons:**", summary["exit_reason_counts"])
        st.write(f"**Expiry-day rolled trades (code 2):** {summary['rolled_trades']}")
        if summary["fill_fallback_count"]:
            st.warning(
                f"{summary['fill_fallback_count']} trade(s) used a fallback "
                "fill (legs missing at the intended minute)."
            )
        if result["signals_skipped"]:
            sk = result["skip_reason_counts"]
            st.warning(
                f"{result['signals_skipped']} signal(s) skipped — "
                f"buy leg not listed: {sk['buy_leg_missing']}, "
                f"sell leg not listed: {sk['sell_leg_missing']}, "
                f"spot missing: {sk['spot_missing']}, "
                f"non-positive credit: {sk['nonpositive_credit']}. "
                "Missing legs are far-OTM strikes absent from the chain that "
                "minute (common on the rolled next-weekly chain on expiry days); "
                "the structure is always a credit, so non-positive credit only "
                "fires on stale/illiquid ticks."
            )

        equity_df = pd.read_csv(result["equity_csv"])
        if not equity_df.empty:
            st.subheader("Equity curve")
            st.line_chart(equity_df["equity_inr"])
            st.subheader("Drawdown")
            st.area_chart(equity_df["drawdown_inr"])

        trades_df = pd.read_csv(result["trades_csv"])
        if not trades_df.empty:
            st.subheader("Trades")
            st.dataframe(trades_df, use_container_width=True)
        else:
            st.info("No trades generated in this window.")

        d1, d2 = st.columns(2)
        with open(result["trades_csv"], "rb") as f:
            d1.download_button(
                "Download trades CSV",
                data=f.read(),
                file_name=os.path.basename(result["trades_csv"]),
                mime="text/csv",
                key="tse_dl_trades",
            )
        with open(result["equity_csv"], "rb") as f:
            d2.download_button(
                "Download equity CSV",
                data=f.read(),
                file_name=os.path.basename(result["equity_csv"]),
                mime="text/csv",
                key="tse_dl_equity",
            )
